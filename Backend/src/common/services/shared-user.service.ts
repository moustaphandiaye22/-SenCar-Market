...
import { ROLES_ADMIN_MODERATION, ROLES_ADMIN_SUPER_ADMIN } from '../constants/role-groups';
...
export class SharedUserService {
   ...
   */
  async getCurrentUser(authenticatedUser: AuthenticatedUser): Promise<SharedUserRecord> {
   ...
   */
  async findByEmailOrTelephone(identifiant: string): Promise<SharedUserRecord | null> {
   ...
   */
  async mustGetCurrentUser(authenticatedUser: AuthenticatedUser): Promise<SharedUserRecord> {
  ...
  async isAdmin(authenticatedUser: AuthenticatedUser): Promise<boolean> {
    const user = await this.getCurrentUser(authenticatedUser);
    return hasAnyRole(user.typeUtilisateur?.nom, ROLES_ADMIN_SUPER_ADMIN);
  }

  /**
   * Check if current user is admin or moderator
   */
  async isAdminOrModerator(authenticatedUser: AuthenticatedUser): Promise<boolean> {
    const user = await this.getCurrentUser(authenticatedUser);
    return hasAnyRole(user.typeUtilisateur?.nom, ROLES_ADMIN_MODERATION);
  }

  /**
   * Ensure current user is admin - throws if not
   */
  async ensureAdmin(authenticatedUser: AuthenticatedUser): Promise<SharedUserRecord> {
    const user = await this.getCurrentUser(authenticatedUser);
    if (!hasAnyRole(user.typeUtilisateur?.nom, ROLES_ADMIN_SUPER_ADMIN)) {
      throw new DomainException('Accès refusé. Droits administrateur requis.', 403, 'ADMIN_REQUIRED');
    }
    return user;
  }
   ...
   */
  async ensureAdminOrModerator(authenticatedUser: AuthenticatedUser): Promise<SharedUserRecord> {
    const user = await this.getCurrentUser(authenticatedUser);
    if (!hasAnyRole(user.typeUtilisateur?.nom, ROLES_ADMIN_MODERATION)) {
      throw new DomainException('Accès refusé. Droits administrateur ou modérateur requis.', 403, 'ADMIN_OR_MODERATOR_REQUIRED');
    }
    return user;
  }
   ...
   */
  async isOwnerOrAdmin(authenticatedUser: AuthenticatedUser, ownerId: string): Promise<boolean> {
    const user = await this.getCurrentUser(authenticatedUser);
