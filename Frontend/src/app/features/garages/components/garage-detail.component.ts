...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, LucideAngularModule, SafePipe],
...
export class GarageDetailComponent implements OnInit {
  ...
  loadGarageData(id: string): void {
    ...
    this.garageService.getGarageById(id).subscribe({
      next: (garage) => {
        ...
        this.isOwner =
          !!currentUser &&
          !!garage.proprietaireId &&
          currentUser.id === garage.proprietaireId;
  ...
  getLogoUrl(url?: string | null): string {
    if (!url)
      return `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(this.garage?.nom || 'Garage')}&backgroundColor=F3F4F6&textColor=1F2937`;
  ...
  getMapEmbedUrl(): SafeResourceUrl | null {
    if (!this.garage?.latitude || !this.garage?.longitude) return null;
    const lat = this.garage.latitude;
    const lng = this.garage.longitude;
    // Use a tighter bounding box (~500m) and include the marker
    const bbox = `${lng - 0.005},${lat - 0.005},${lng + 0.005},${lat + 0.005}`;
    const url = `https://www.openstreetmap.org/export/embed.html?bbox=${bbox}&layer=mapnik&marker=${lat}%2C${lng}&zoom=16`;
    return this.sanitizer.bypassSecurityTrustResourceUrl(url);
  }

  getGoogleMapsUrl(): string {
    if (!this.garage?.latitude || !this.garage?.longitude) return '';
    return `https://www.google.com/maps/dir/?api=1&destination=${this.garage.latitude},${this.garage.longitude}`;
  }
  ...
  deleteGarage(): void {
    this.confirmService.show({
      ...
      message:
        'Êtes-vous sûr de vouloir supprimer ce garage ? Cette action est irréversible.',
  ...
  onWhatsAppContact(): void {
    ...
    if (phone) {
      ...
      const formattedPhone =
        cleanPhone.length === 9 ? '221' + cleanPhone : cleanPhone;

      const message = encodeURIComponent(
        `Bonjour, je vous contacte via Sen-Car Market pour une demande de service dans votre garage ${this.garage?.nom}.`,
      );
        ...
        this.garageService.createRendezVous(request).subscribe({
          next: () => {
            this.toastService.success(
              'Votre demande de rendez-vous a été envoyée ! Vous recevrez une notification prochainement.',
            );
          },
          error: (err) => {
