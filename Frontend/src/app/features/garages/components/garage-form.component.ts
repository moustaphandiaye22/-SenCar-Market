...
import { LucideAngularModule, MapPin, Phone, Building, Info, Save, X, Plus, Trash2, Clock, Mail, Globe, Upload, ChevronRight, ChevronLeft, CheckCircle } from 'lucide-angular';
...
export class GarageFormComponent implements OnInit {
  ...
  canGoNext(): boolean {
    if (this.currentStep === 1) {
      return this.garageForm.get('nom')!.valid && 
             this.garageForm.get('description')!.valid;
    }
    if (this.currentStep === 2) {
      return this.garageForm.get('telephone')!.valid && 
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      if (this.selectedLogoFile) {
        const uploadRes = await this.garageService.uploadLogo(this.selectedLogoFile).toPromise();
        if (uploadRes) {
      ...
      this.errorMessage = 'Une erreur est survenue lors du chargement de l\'image.';
      this.isLoading = false;
    }
  }
}
