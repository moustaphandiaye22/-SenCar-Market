...
export class RentalService {
  ...
  getAllAnnonces(page = 0, size = 12): Observable<any> {
    const params = new HttpParams()
      .set('page', page.toString())
      .set('size', size.toString());
    return this.http.get<any>(`${this.apiUrl}/annonces`, { params });
  }

  // Keep the old method for backwards compatibility
  getAllAnnoncesLegacy(): Observable<AnnonceLocation[]> {
  ...
  createReservation(request: any): Observable<ReservationLocation> {
    return this.http.post<ReservationLocation>(
  ...
  getMesReservations(): Observable<ReservationLocation[]> {
    return this.http.get<ReservationLocation[]>(
      `${this.apiUrl}/mes-reservations`,
    );
  }

  getDisponibilites(annonceId: string): Observable<DisponibiliteLocation[]> {
    return this.http.get<DisponibiliteLocation[]>(
      `${this.apiUrl}/annonces/${annonceId}/disponibilites`,
    );
  }
  ...
  updateAnnonce(id: string, request: any): Observable<AnnonceLocation> {
    return this.http.put<AnnonceLocation>(
  ...
  cancelReservation(id: string, reason?: string): Observable<any> {
    return this.http.post(`${this.apiUrl}/reservations/${id}/annuler`, {
      motif: reason,
    });
  }
  ...
  activerAnnonce(id: string): Observable<AnnonceLocation> {
    return this.http.post<AnnonceLocation>(
      `${this.apiUrl}/annonces/${id}/activer`,
      {},
    );
  }
  ...
  desactiverAnnonce(id: string): Observable<AnnonceLocation> {
    return this.http.post<AnnonceLocation>(
      `${this.apiUrl}/annonces/${id}/desactiver`,
      {},
    );
  }
  ...
  updateStatutReservation(
    ...
    return this.http.put<ReservationLocation>(
  ...
  getReservationsByAnnonce(id: string): Observable<ReservationLocation[]> {
    return this.http.get<ReservationLocation[]>(
      `${this.apiUrl}/annonces/${id}/reservations`,
    );
  }
  ...
  ajouterDisponibilites(
    id: string,
    request: any[],
  ): Observable<DisponibiliteLocation[]> {
    return this.http.post<DisponibiliteLocation[]>(
      `${this.apiUrl}/annonces/${id}/disponibilites`,
      request,
    );
  }
  ...
  updateStatutReservationAvecHistorique(
    ...
    const params = new HttpParams().set('statut', statut);
    return this.http.put<ReservationLocation>(
      `${this.apiUrl}/reservations/${id}/statut-avec-historique`,
      {},
      { params },
    );
  }
